...
- Full pipeline (Styx delta + enrichment): `<hermes-install>/.venv/bin/python3.13 <hermes-home>/skills/ocas-taste/scripts/taste_full_enrich.py --limit 200`
- Email-only historical scan: `<hermes-install>/.venv/bin/python3.13 <hermes-home>/skills/ocas-taste/scripts/taste_scan.py scan-historical 365`
...
- Signal dedup: `<hermes-install>/.venv/bin/python3.13 <hermes-home>/commons/data/ocas-taste/scripts/taste_signals_dedup.py` — deduplicates signals after enrichment runs. **Takes no arguments** — runs against the default data path. Confirmed working 2026-06-18 (0 dupes found on 4,056 signals; 46 dupes removed on prior run). Must be run from the data directory or with `cd <hermes-root>/commons/data/ocas-taste` first.
- Signal cleanup (generic meal titles): `<hermes-install>/.venv/bin/python3.13 <hermes-home>/skills/ocas-taste/scripts/clean_signals.py <hermes-root>/commons/data/ocas-taste/signals.jsonl` — removes generic meal titles (Breakfast, Lunch, Dinner, Brunch) and deduplicates on `(venue_name, event_date, extraction_source, domain)`. On 2026-06-16 it removed 5,605 duplicate signals (9,310 → 3,705).
...
```bash
<hermes-install>/.venv/bin/python3.13 <hermes-home>/skills/ocas-taste/scripts/taste_scan.py scan-incremental 24
```
