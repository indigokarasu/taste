...
```python
token_paths = [
    Path.home() / ".hermes" / "owner_google_credentials.json",   # the operator (user)
...
The user signs in with **google-workspace-user** (the account with 248K
...
token_paths = [
    ...
    Path.home() / ".hermes" / "owner_google_credentials.json",   # owner (user)
]
