...
cd <hermes-root>/commons/data/ocas-taste && \
  <hermes-root>/commons/data/ocas-taste/venv/bin/python3 \
  <hermes-root>/skills/ocas-taste/scripts/taste_scan.py \
  scan-historical 1
```

Expected output: `Initialized Gmail and Calendar with google-workspace-user.json` followed by message processing counts.
