...
The helper handles token refresh automatically. Credentials are stored at `/root/.google_workspace_mcp/credentials/`. If tokens are revoked, re-authorize with `python3 <hermes-root>/skills/infrastructure/google-workspace-auth/scripts/google_oauth_init.py`.
