...
- `taste.update` — retired; skill updates run fleet-wide via the centralized `skills:update-fleet` cron (`scripts/update_skill.sh`), never an in-place `git pull`
...
- **`scan-historical` output needs a schema-aware dedup** — Any dedup key that reads `name`/`normalized_name` silently skips scan signals (they use `venue_name`), reporting a false "0 dupes" on `--dry-run`. Use `scripts/clean_signals.py` (keys on `venue_name` / `event_date[:10]` / source / domain) or `scripts/safe_taste_dedup.py` (keys on `event_date or date`). Combined with the date bug above, re-running `scan-historical` over an already-populated dataset silently pollutes it with mis-dated signals — verify against the actual signal schema, not a tool's count, and revert if dates are wrong.
